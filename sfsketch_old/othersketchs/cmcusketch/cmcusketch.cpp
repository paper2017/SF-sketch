...
CMCUSketch::CMCUSketch(uint d_, uint w_, uint counterBits_) {
...
inline uint CMCUSketch::getCounter(uint i, uint j) {
...
inline void CMCUSketch::setCounter(uint i, uint j, uint val) {
    // Conform the same convention in CMCUSketch::getCounter
...
void CMCUSketch::insert(const uchar *str, uint len, uint r) {
    uint min = maxCounter;
//----------------------------------
//    static int atest = 0;
//----------------------------------
...
//----------------------------------
    ...
    for (uint i = 0; i < d; ++i) {
        if ( min > auxis[i].c) {
            setCounter(i, auxis[i].w, min);
        }
    }
}

// Query str's counter
uint CMCUSketch::queryPoint(const uchar *str, uint len) {
	uint ret = maxCounter;
	for (uint i = 0; i < d; ++i) {
		uint pos = h[i].run(str, len) % w;
		uint counter = getCounter(i, pos);
