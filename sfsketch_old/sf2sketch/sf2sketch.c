...
//for right2:
    ...
    length = WL;
#endif
    total = sizeof(sketch_r2) + sketch.D * (length * sizeof(size_t) + sizeof(void *));
    if (!(sketch.right2 = (sketch_r2 *)malloc(total))) {
        dbg_printf("init: malloc failed!");
        return false;
    }
    memset(sketch.right2, 0, total);
    sketch.right2->D = D;
    sketch.right2->W = WL;
    sketch.right2->bits_c = bits_c;
    sketch.right2->max_c = sketch.max_c;
    sketch.right2->length = length;
    for (int i=0; i<sketch.D; i++) {
        sketch.right2->counts[i] = (size_t *)(&sketch.right2->counts[sketch.D]) + i * length;
    }
...
static size_t get_r2c(size_t d, size_t w)
{
#ifdef COMPACT_COUNTER_R
    return get_cpt_c(d, w, sketch.right2->counts);
#else
    return sketch.right2->counts[d][w];
#endif
}
...
static void set_r2c(size_t d, size_t w, size_t val)
{
#ifdef COMPACT_COUNTER_R
    set_cpt_c(d, w, val, sketch.right2->counts);
#else
    sketch.right2->counts[d][w] = val;
#endif
}
...
static void inc_r2(const unsigned char * str, size_t len, size_t delta)
{
    size_t d = 0, counter;
    for (d = 0; d < sketch.D; ++d) {
        sketch.auxis[d].w = hash(d, str, len) % sketch.right2->W;
        counter = get_r2c(d, sketch.auxis[d].w);
        counter = (((sketch.right2->max_c - delta) > counter) ? (counter + delta) : sketch.right2->max_c);
        set_r2c(d, sketch.auxis[d].w, counter);
    }
}
...
static void dec_r2(const unsigned char * str, size_t len, size_t delta)
{
    size_t d = 0;
    for (d = 0; d < sketch.D; ++d) {
        sketch.auxis[d].w = hash(d, str, len) % sketch.right2->W;
        sketch.auxis[d].c = get_r2c(d, sketch.auxis[d].w);
        (sketch.auxis[d].c > delta) ? (sketch.auxis[d].c -= delta) : (sketch.auxis[d].c = 0);
        set_r2c(d, sketch.auxis[d].w, sketch.auxis[d].c);
    }
}
