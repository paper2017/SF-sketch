...
#for test:
#sh experiment_zipf_inc-dec.sh 5 40000 3 24 4 1

#we create 100K items
BASE_ITEMS_K=100
BASE_OP_K=100
BASE_OP=100000
TEMPLET=./templets/kv100K_opXXXK_zipf.dat
...
if     [ "$#" -ne 6 ] || [ "$W" -lt 1 ] || [ "$D" -lt 1 ]\
    ...
    || [ "${END}" -lt 1 ] || [ "${DELTA}" -lt 1 ]
then
    echo -e We consider `RED 100K` items with operation frequents obeying `RED "zipfian distribution"`
    ...
    echo -e usage: ./experiment_zipf_inc-dec.sh `COLOR D` `COLOR W` `COLOR K` `COLOR BITS_C`  `COLOR END` `COLOR DELTA`
    exit
else
    echo -e We consider `RED 100K` items with operation frequents obeying `RED "zipfian distribution"`
    echo -e record the frequency of each item in every `COLOR ${DELTA}*100K` operations.
...
EXP_DIR=${CUR_DIR}/raw_data_experiment/kv${BASE_ITEMS_K}K_${END}_${DELTA}_zipf_inc-dec_${D}_${W}_${K}_${BITS_C}
