...
#sh experiment_uniform_dec.sh 5 40000 3 24 11 8 1
...
TEMPLET=${CUR_DIR}/templets/kv100K_opXXXK_uniform.dat
...
#    echo -e `RED 'make sure START>=10 END >=5 START >= END'`
    ...
    echo -e We experiment `RED 100K` items with frequents `RED "uniformly distributed"`
    ...
    EXP_DIR=${CUR_DIR}/raw_data_experiment/kv${BASE_ITEMS_K}K_${START}_${END}_${DELTA}_uniform_dec_${D}_${W}_${K}_${BITS_C}
    if [ ! -d $EXP_DIR ];then
        mkdir $EXP_DIR
    fi
    echo $SETTING_FILE
...
then
    ...
    mv $sketch_DIR/sfsketch.out $EXP_DIR/${SETTING_BASE}_sfsketch.out
    for ((i=$START-$DELTA,j=0;i>=$END;i-=$DELTA));do
        ((j++))
        CHSETTING $DELTA dec$i
