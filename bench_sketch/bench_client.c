...
//         ret = memc_init_inc(memc, key, 0, delta, &value);
...
//ret = memc_inc(memc, key, delta, &value);
      ...
      for (size_t i = p->num_ops; i >= 1; ) {
          ...
          if (type == request_inc) {
              ...
              fprintf(stderr, "unknown request type: %ld\n", i);
          }
      }
  }
    fclose(fout);
...
{
  printf("%s [-c IP] [-t #] [-b #] [-l trace] [-d #] [-h] [-s IP] [-r]\n", binname);
...
main(int argc, char **argv)
{
  ...
  char ch;
  while ((ch = getopt(argc, argv, "t:b:d:s:l:w:rh")) != -1) {
    switch (ch) {
