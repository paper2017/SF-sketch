                 ...
                 buffer_len)
{
    ...
    printf("Total Num Bytes = %" PRIu64 ", sizeof qfblock = %d, qfmetadata = %d\n", total_num_bytes, sizeof(qfblock), sizeof(qfmetadata));
...
int qf_insert_s(QF *qf, const unsigned char * key, size_t len, uint64_t count)
{
    // We fill up the CQF up to 95% load factor.
    // This is a very conservative check.
    if (qf_get_num_occupied_slots(qf) >= qf->metadata->nslots * 0.95) {
    ...
    _key = MurmurHash64A(key, len,
                         qf->metadata->seed) % qf->metadata->range;
    uint64_t hash = (_key << qf->metadata->value_bits);
    int ret;
...
uint64_t qf_count_key_value_s(const QF *qf, const unsigned char * key, size_t len)
{
    uint64_t _key;
            _key = MurmurHash64A(key, len,
                                qf->metadata->seed) % qf->metadata->range;
