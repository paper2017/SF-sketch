...
static void test_thrpt(request *requests, CMLSketch & c){
  struct timeval tv_s, tv_e;
  double time;
  FILE *ftime = fopen("throughput_cmlsketch.out", "w");
...
static void requests_exec(request *requests, CMLSketch & c, char *filename)
{
...
static void usage(char* binname)
{
  ...
  printf("\t-d # : number of tables on/off chip in a cmlsketch, required\n");
...
int main(int argc, char **argv)
{
  ...
  CMLSketch c(D, W*4, Bits_c, 1.08, 255);
  request *requests = NULL;

  char finit_up[100] = "init_up";
  char finc_dec_up[100] = "inc-dec_up";
  char fquery[] = "query";
  char foutput[100] = "cmlsketch.out";
  char foutput2[100] = "cmlsketch_r.out";
    ...
    for (int i=1; i<=nSubfiles; i++){
      ...
      sprintf(foutput,"cmlsketch%d.out",i);
      requests = requests_init(finc_dec_up);
      requests_exec(requests, c, foutput);
      free(requests);
      requests = NULL;
