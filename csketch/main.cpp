...
static void test_thrpt(request *requests){
  ...
  double time;
  FILE *ftime = fopen("throughput_csketch.out", "w");
...
static void requests_exec_r()
{
  ...
  while (index < loads.num) {
    ...
    snprintf(outputfile, sizeof(outputfile), "csketch_%d.out", count);
    queryfqs(outputfile);
  }
  index = loads.num - 1;
  while (index >= 0) {
    ...
    snprintf(outputfile, sizeof(outputfile), "csketch_r_%d.out", count);
    queryfqs(outputfile);
    ops = interval;
  }
  printf("requests_exec...done\n");
...
{//"usage: ./bench_update_gen  output_filename input_filename check_filename append_or_not[yes/no]"
  ...
  printf("\t-d # : number of tables on/off chip in a csketch, required\n");
    ...
    requests_exec_r();
     ...
     requests = NULL;
      ...
      for (int i=1; i<=nSubfiles; i++){
          ...
          sprintf(foutput,"csketch%d.out",i);
          requests = requests_init(finc_dec_up);
          requests_exec(requests, foutput);
          free(requests);
          requests = NULL;
