...
static void test_thrpt(request *requests) {
    ...
    double time;
    FILE *ftime = fopen("throughput_sfsketch.out", "w");
...
static void requests_exec_r()
{
    ...
    while (index < loads.num) {
        ...
        snprintf(outputfile, sizeof(outputfile), "sfsketch_%d.out", count);
        queryfqs(outputfile);
    }
    index = loads.num - 1;
    while (index >= 0) {
        ...
        snprintf(outputfile, sizeof(outputfile), "sfsketch_r_%d.out", count);
        queryfqs(outputfile);
        ops = interval;
    }
    printf("requests_exec...done\n");
...
{   //"usage: ./bench_update_gen  output_filename input_filename check_filename append_or_not[yes/no]"
    ...
    printf("\t-d # : number of tables on/off chip in a sfsketch, required\n");
...
int main(int argc, char **argv)
{
    ...
    char fquery[] = "query";
    char foutput[100] = "sfsketch.out";  //for query in slim sketch using aging
#ifdef SF_AGING
    //char foutput_fat[100] = "sfsketch_fat.out";  //for test , compare with slim query.
#endif
...
#endif
    {
        ...
        for (int i=1; i<=nSubfiles; i++) {
            ...
            sprintf(foutput,"sfsketch%d.out",i);
            //sprintf(foutput_fat,"sfsketch_fat%d.out",i);
            requests = requests_init(finc_dec_up);
            requests_exec(requests, foutput);
            free(requests);
            requests = NULL;
